<template>
    <div class="h-full flex flex-col">
        ...
        <div v-if="tracks.length"
            ...
            <RecycleScroller class="scroller h-full pr-1" :items="tracks" :item-size="64" key-field="path"
                ...
                <SongRow :track="track" :index="index" :is-active="currentTrack?.path === track.path"
                    @play="playTrack(track)" @toggleFavorite="toggleFavorite(track)" />
            </RecycleScroller>

            <div class="mt-2 text-center text-xs text-gray-500 shrink-0 pb-2">
                Total: {{ tracks.length }} canciones favoritas
            </div>
        </div>
        ...
        <div v-else-if="!loading" class="flex-1 flex flex-col items-center justify-center text-base-content/50">
            ...
            <p class="text-lg font-medium">No tienes canciones favoritas.</p>
            <p class="text-sm mt-2">Escucha música y marca canciones como favoritas para agregarlas aquí.</p>
        </div>
...
async function loadTracks() {
    loading.value = true
    try {
        const data = await invoke("get_favorites", {
